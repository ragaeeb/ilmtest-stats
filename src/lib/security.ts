...
/**
 * Initializes the encryption key using the provided secret or the `ENCRYPTION_SECRET`
 * environment variable. Subsequent calls are no-ops once the key is set.
 *
 * @param encryptionSecret - Optional override for the secret used to derive the key.
...
/**
 * Encrypts the provided UTF-8 string using AES-256-GCM and returns a base64url token.
 *
 * @param plaintext - The value to encrypt.
 * @returns The encrypted payload encoded as a base64url string.
 */
...
/**
 * Decrypts a token generated by {@link encrypt} and returns the original plaintext.
 *
 * @param token - The base64url encoded ciphertext.
...
/**
 * Resets the cached encryption key. This is intended for unit tests that need to
 * exercise error branches by forcing {@link initSecrets} to derive a new key.
 */
export const resetSecretsForTests = () => {
    KEY = null;
};
